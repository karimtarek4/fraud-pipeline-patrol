...
def generate_customers(start_id=1001, num_customers=1000):
    """
    Build a customer profile table with:
      - Behavioral baselines (AvgTransactionAmount, PastFraudCount, etc.)
      - Home device/IP & geolocation
      - batch_id & ingestion_date for incremental pipeline
    """
    # Base demographics
    df = pd.DataFrame({
    ...
    # choice: probability 
    df["PastFraudCount"]       = np.random.choice([0, 1, 2], num_customers, p=[0.9, 0.08, 0.02])
    # for _ in: number of times 
    df["HomeDevice"]           = [f"dev_{np.random.randint(1000,9999)}" for _ in range(num_customers)]
    df["HomeIP"]               = [f"192.168.{np.random.randint(0,255)}.{np.random.randint(0,255)}"
...
def generate_merchants(start_id=2001, num_merchants=1000):
    """
    Build a merchant table with:
      - Category & MerchantRiskScore (Risk of category)
      - ingestion_date
    """
...
def random_timestamps(start, count, scale_seconds=3600):
    """
    Generate `count` timestamps from `start` using exponential inter-arrival times.
    """
...
def generate_transactions(customers, merchants, avg_txs_per_cust=5.0):
    ...
    Build a transactions table with rich features:
      ...
      - Channel, PastFraudCount, Merchant lookup, FailedTransactions
    """
    records = []
    tx_id = 1
    ingestion_date = return_current_ingestion_date()
    ...
    for _, cust in customers.iterrows():
        ...
        for ts in random_starts:
            ...
            ip     = cust["HomeIP"] if np.random.rand() < 0.9 \
...
def generate_login_attempts(transactions):
    """
    Pull each failed failed into its own record with its own timestamp.
    """
    logs = []
