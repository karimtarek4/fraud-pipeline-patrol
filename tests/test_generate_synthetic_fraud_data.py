...
from scripts.generate_synthetic_fraud_data import (
    generate_customers,
    generate_merchants,
...
class TestDataGenerationLogic:
    ...
    def test_generate_customers_field_validity(self):
        ...
        - Checks that each field meets business requirements
        
        Developer Notes:
        - Validates data types, ranges, and business logic
        - Ensures CustomerID sequence is correct
        - Checks that behavioral fields are reasonable
        """
        ...
        assert actual_ids == expected_ids, f"CustomerID sequence incorrect: {actual_ids}"
        ...
        assert avg_amounts.min() >= 50, f"Min transaction amount {avg_amounts.min()} below 50"
        assert avg_amounts.max() <= 200, f"Max transaction amount {avg_amounts.max()} above 200"
        
        # Test PastFraudCount values (should only be 0, 1, or 2)
        fraud_counts = customers["PastFraudCount"].unique()
        valid_counts = {0, 1, 2}
        assert set(fraud_counts).issubset(valid_counts), f"Invalid fraud counts: {fraud_counts}"
        
        # Test geographic coordinates (business requirement: reasonable US coordinates)
        lats = customers["HomeLat"]
        lons = customers["HomeLon"]
        assert lats.min() >= 30.0 and lats.max() <= 45.0, f"Latitude out of range: {lats.min()}-{lats.max()}"
        assert lons.min() >= -100.0 and lons.max() <= -70.0, f"Longitude out of range: {lons.min()}-{lons.max()}"
        
        # Test required fields exist
        required_fields = ["CustomerID", "Age", "HomeDevice", "HomeIP", "ingestion_date"]
    ...
    def test_generate_merchants_logic(self):
        ...
        risk_map = {"Groceries":0.1,"Retail":0.2,"Travel":0.15,
                   "Online":0.25,"Gambling":0.8,"Crypto":0.7}
...
class TestFileOperations:
    ...
    def test_ensure_dirs_creation(self, mock_makedirs):
        ...
        assert mock_makedirs.call_count == 3, f"Expected 3 calls to makedirs, got {mock_makedirs.call_count}"
    ...
    def test_main_file_writing(self, mock_resolve, mock_ensure_dirs, mock_to_parquet):
        ...
        for expected_file in expected_files:
            assert any(expected_file in call_arg for call_arg in call_args), \
...
class TestDataIntegrity:
    ...
    def test_data_relationship_integrity(self):
        ...
        assert customer_ids_transactions.issubset(customer_ids_customers), \
        ...
        assert merchant_ids_transactions.issubset(merchant_ids_merchants), \
        ...
        for i in range(1, len(ingestion_dates)):
            ...
            assert time_diff < 10, f"Ingestion dates should be close, but differ by {time_diff} seconds"
